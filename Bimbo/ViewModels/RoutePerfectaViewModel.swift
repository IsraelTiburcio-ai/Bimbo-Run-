...
final class RoutePerfectaViewModel {
    ...
    var daySummaryText: String {
        ...
        let topItems = truckInventory
            ...
            .map(\.product.name)
            .joined(separator: ", ")
        return "Buenos días. Tienes \(pendingCount) tiendas pendientes y \(String(format: "%.1f", totalKm)) kilómetros estimados. Primera parada: \(firstStop). Enfócate en revisar anaquel, caducidades y surtir solo lo necesario. Inventario fuerte: \(topItems)."
    }
